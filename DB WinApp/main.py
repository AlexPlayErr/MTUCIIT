...
class Window(QWidget):  
    ...
    def delete_day_row(self, rowNumb,tname):
        ...
        print(row)   
        try: 
            self.cursor.execute("DELETE FROM timetable WHERE day=%s AND subject=%s AND room_num=%s AND start_time=%s AND id=%s",(str(row[1]),str(row[2]),str(row[3]), str(row[4]),int(row[0])))  
            self.conn.commit()  
            self.update_day_tables() 
        except: 
            QMessageBox.about(self, "Error", "Smth went wrong. Check table and try again")
    ...
    def delete_teacher_row(self, rowNumb,tname):
        ...
        print(row)   
        try: 
            self.cursor.execute("DELETE FROM teacher WHERE full_name=%s AND subject=%s AND id=%s",(str(row[1]),str(row[2]),int(row[0])))  
            self.conn.commit()   
        except: 
            QMessageBox.about(self, "Error", "Smth went wrong. Check table and try again")
        self.update_teacher_table()
